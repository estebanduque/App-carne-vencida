...
// Una tienda marcada como silenciada en la tabla 'lugares' se sigue reconociendo en la
// app (la franja te muestra que te falta si entras) pero nunca dispara el push: es la
// que tenes pegada a casa o al trabajo, donde el GPS no puede separar una cosa de la
// otra y el aviso seria ruido todos los dias.
...
Deno.serve(async (req: Request) => {
  try {
    ...
    // Se piden todas las columnas a proposito: asi la funcion sigue respondiendo aunque
    // 'silenciada' todavia no exista en la tabla, en vez de tirar 500 y dejarte sin avisos.
    ...
    const dentro = (lugares ?? [])
      ...
      .filter((l) => !l.silenciada && l.distancia <= (l.radio ?? 120));

    if (!dentro.length) {
      return Response.json({ ok: true, coincide: false, enviadas: 0 });
    }

    // Todas las columnas, por lo mismo que arriba: 'categorias' puede no existir todavia.
    const { data: pendientes, error: errC } = await supabase
      .from("compras").select("*").eq("comprado", false);
    if (errC) throw errC;

    // Un articulo puede conseguirse en varias tiendas: el pan brioche esta en la
    // panaderia y en el super, el pegamento en la ferreteria y en el super. 'categoria'
    // queda como la principal y es el respaldo si la lista no esta cargada.
    const tiendasDe = (c: Record<string, unknown>): string[] => {
      const l = Array.isArray(c.categorias) ? (c.categorias as string[]).filter(Boolean) : [];
      return l.length ? l : (c.categoria ? [c.categoria as string] : []);
    };
    ...
    const faltanEn = (categoria: string) => {
      ...
      return (pendientes ?? []).filter((c) => tiendasDe(c).some((t) => cubre.includes(t)));
    };
    dentro.sort((a, b) =>
